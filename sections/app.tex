...
\Cref{fig:route-fit-bike-paths} shows opinions given on the in-app bike routing in relation to each given SUS. Respondents for which no SUS could be calculated are highlighted in gray. While there is a less distinguished opinion on the route's fit with bike paths, improvement potentials can be seen with the routes' fit to normally taken paths. The route planning's intuitiveness and clarity seem to have had an impact on the overall usability, as seen by the SUS distribution. However, one user reporting a low SUS seems to have liked the routing, together with the main group of high-SUS respondents. The speed advisory coverage seems to be incorporated by some users into their route planning process.
