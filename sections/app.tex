...
- Studien zu Impacts können in drei Aspekte aufgeteilt werden: Quantitative Messungen der Energieersparnis, gezeigt in \Cref{fig:related-work-energy-savings}, Quantitative Messungen der Haltereduktion an Kreuzungen, und die qualitativen Auswirkungen auf das subjektive Empfinden

- Zur Messung der Energieersparnis wird meist entweder direkt der Spritverbrauch gemessen, oder als Proxy die CO2-Emissionen. Es wird davon ausgegangen, dass die CO2-Emissionen linear mit dem Spritverbrauch zusammenhängen \cite{seredynski_comparison_2013, seredynski_multi-segment_2013}. Eine relative Ersparnis ohne und mit GLOSA sollte sich also vergleichen lassen, auch wenn sie natürlich vom experimentellen Setup abhängt.
...
- Die tatsächliche Energieersparnis bei der Anwendung des Empfehlungssystems in alltäglichen Situationen liegt für Autos und größere motorisierte Fahrzeuge deutlich darunter
- Die höchste direkt an einem echten Fahrzeug gemessene Einsparnis sind 22.1\%, die von Chen et al. (2022) \cite{chen_developing_2022} gemessen wurden. Hierfür wurde aber auch ohne Verkehr mit einem Bus über eine Teststrecke mit 2 Testampeln gefahren. In einer anderen Teststrecken-Studie von Hao et al. (2019) wurden 6\% \cite{hao_eco-approach_2019} Ersparnis gemessen.
- Evaluationen direkt im Straßenverkehr kommen zu Einsparnissen von 20.3\% \cite{koukoumidis_signalguru_2011} bzw. 13.59\% \cite{xia_field_2012}. Allerdings gibt es hiervon generell nur wenige Studien. Meist werden auch nur wenige bzw. eine einzelne Kreuzung berücksichtigt, dadurch keine wirklich alltägliche Fahrten untersucht
- Xu et al. (2015) \cite{xu_bb_2015} kommen zu 15.1\% Ersparnissen beim Test mit 3 Kreuzungen in Bejing, aber die Tests wurden bei Nacht ausgeführt um eine Interferenz mit anderen Fahrzeugen zu vermeiden.
- Khan et al. (2021) \cite{khan_eco-drive_2021} messen eine Energieersparnis von 5\% beim Test mit 1187 Kreuzungen in Ottawa, Canada. Beim Test wurden 830 Fahrzeuge mit einer GLOSA-App ausgestattet und über 8 Wochen aufgezeichnete Fahrtdaten ausgewertet.

- Für elektrische Fahrräder messen Tal et al. (2016) \cite{tal_vehicular-communications-based_2016} eine Ersparnis von 32 - 46\% per iTETRIS (SUMO) Simulation, getestet wurde eine Kreuzung. Die Energieersparnis wurde durch ein Energieverbrauchsmodell gemessen, welches den überwundenen Luftwiderstand, Neigung der Straße, und den Rollwiderstand berücksichtigt. Ein ähnliches Modell benutzen auch Dabiri et al. (2020) \cite{dabiri_optimized_2020}, nur, dass hier noch die Beschleunigung mit einfließt
- Die tatsächliche Energieersparnis bei alltäglichen Fahrten wurde bisher bei Fahrradfahrern nicht gemessen.
...
- Zweiter untersuchter Impact: Reduktion der Stop time bzw. stop rate wie in \Cref{fig:related-work-stops} gezeigt.
- Die Stop time reduction wurde bisher fast ausschließlich in Simulationen gemessen. Karoui et al. (2018) \cite{karoui_efficiency_2018}, Sharara et al. (2019) \cite{sharara_impact_2019} und Xie et al. (2021) \cite{xie_dynamic_2021} kommen auf eine Reduktion von bis zu 100\% in günstigen Bedingungen, also eine vollständige Vermeidung des Stehenbleibens an der Kreuzung.
- Die meisten Messungen liegen aber deutlich darunter, bei einem Median von 61\% (IQR: 61\%). Auch hier ist es wahrscheinlich, dass die tatsächliche Einsparung der Haltezeit in der Realität deutlich geringer ausfällt.
- Bisher gibt es dafür aber nur einen Datenpunkt von Suramardhana et al. (2014) \cite{suramardhana_driver-centric_2014}. Hier wurde bei der Zufahrt auf eine Teststrecken-Kreuzung in Korea eine Haltezeitreduktion von 23.9\% gemessen.
- Zur Stopratenreduktion gibt es auch eine Handvoll Messungen, die im Median bei 45\% (IQR: 30\%) liegen. Direkt im Feld gemessen wurde aber nur in der Studie von Xu et al. (2015) \cite{xu_bb_2015} eine Reduktion von 21 auf 15 Stops (28.6\% Reduktion). Zu Berücksichtigen ist hierbei wieder, das bei Nacht gemessen wurde, und dass generell nur wenige Fahrten durchgeführt wurden.
- Fickas et al. (2019) \cite{fickas_fast_2019}: Stop rate bei Fahrradfahrern nicht direkt gemessen, aber in den meisten Fällen konnte die App die Fahrradfahrer erfolgreich bei grün über die Ampel lotsen
- Wie stark Halte bei alltäglichen Fahrten und über mehrere tausend Ampeln mithilfe einer GLOSA reduziert werden können, ist also noch weitestgehend offen, nicht nur für Fahrradfahrer

- Subjective Impacts of the speed advisory
- Khan et al. (2021) \cite{khan_eco-drive_2021} untersuchen neben quantitativen Auswirkungen auch die subjektive Auswirkung und Compliance mit der Geschwindigkeitsempfehlung
- Auf Basis der Befragung von 100 Nutzern wurde die Geschwindigkeitsempfehlung mehrheitlich (70 mal) als "somewhat useful" eingestuft. 53 Befragte gaben an, die Geschwindigkeitsempfehlung mache das Autofahren more relaxed, während 35 keinen Unterschied zur Normalsituation angaben und 12 Probanden sogar eine ängstlicher (anxious) macht. Die meisten Probanden beschrieben, dass sie sich ca. 25\% (47) oder 50\% (29) der Fälle an die Geschwindigkeitsempfehlung angepasst hatten, wobei sich 12 Probanden auch gar nicht an die Geschwindigkeitsempfehlung angepasst haben. In 232 (9.9\%) von 2354 Fällen wird vermutet, dass der umliegende Verkehr eine Anpassung verhindert hat, da die Ampel grün war aber das Fahrzeug auf unter 20 km/h abgebremst wurde
- Der gleiche Fall wurde auch in der fahrradbezogenen Studie von Fickas et al. (2019) \cite{fickas_fast_2019} von Probanden berichtet: andere Fahrradfahrer "verstopften" (clogging) die Radfahrspur, weshalb sich nicht an die Geschwindigkeitsempfehlung gehalten werden konnte, ohne gefährlich auszuweichen. Wie die generelle Nützlichkeit der App beurteilt wurde oder ob der Komfort verbessert werden konnte, wurde nicht abgefragt. Klar ist jedoch, dass einige Probanden gelegentlich das Gefühl hatten, aus der Komfort-Zone gedrängt zu werden um der für sie zu langsamen oder zu schnellen Geschwindigkeitsempfehlung zu folgen.

- Die Effektivität der Geschwindigkeitsempfehlung hängt nicht nur von der subjektiven Bereitschaft ab, dieser zu folgen, sondern auch von vielen weiteren Kontextfaktoren. Sokolov et al. (2018) \cite{sokolov_effects_2018} führen daneben beispielsweise auch die Auswirkung auf andere Verkehrsteilnehmer als Einflussfaktor auf.
- Preuk et al. (2016 -- 2018) \cite{preuk_does_2016, preuk_should_2018} beschäftigen sich mit dieser Frage und kommen zu dem Schluss, dass andere Verkehrsteilnehmer ein durch die Geschwindigkeitsempfehlung angepasstes Fahrzeug als störend wahrnehmen, sofern sie nicht darüber informiert sind, dass dieses Fahrzeug eine Geschwindigkeitsempfehlung nutzt.
- Generell funktioniert die Geschwindigkeitsempfehlung besser, je mehr Fahrzeuge eine solche Geschwindigkeitsempfehlung nutzen (Penetration Rate). Bhattacharyya et al. (2022) \cite{bhattacharyya_assessing_2022}: Energy/fuel savings 3\% - 5\% (MPR 30\%), 10\% (MPR 50\%), 15\% (MPR 100\%); Stop time reduction 3\% (30\% MPR), 62\% (500m), 75\% (1000m)

- Auch die Aktivierungsdistanz und die Transmissionsrate der Prognosenachrichten spielt eine große Rolle bei der Effektivität der Geschwindigkeitsempfehlung, wie Sharara et al. (2019) \cite{sharara_impact_2019} zeigen. Meist wird eine Aktivierungsdistanz im Bereich 200 bis 300 Meter angenommen \cite{xia_indirect_2011, raubitschek_predictive_2011, rakha_eco-driving_2011, rakha_aeris_2012, mahler_reducing_2012, tal_vehicular-communications-based_2016, xu_bb_2015, de_nunzio_eco-driving_2015, luo_green_2017, kloeppel_performance_2019, dabiri_optimized_2020, hu_lane-level_2023}. Dies deckt sich ungefähr mit Messungen aus der echten Welt \cite{bernais_design_2016}, obwohl die Aktivierungsdistanz ohne Relaying \cite{stahlmann_multi-hop_2017} Übertragungs- oder Baubedingt auch deutlich darunter liegen kann \cite{hao_eco-approach_2019}.
- Manche Studien gehen aber auch in Teilen von Szenarien mit über 1000 Meter Anfahrtsweg aus \cite{tielert_impact_2010, asadi_predictive_2011, krajzewicz_preparing_2012, nguyen_efficient_2016, karoui_efficiency_2018, plianos_predictive_2018, sharara_impact_2019, simchon_real-time_2020, bhattacharyya_assessing_2022}.

- Abschließend lässt sich sagen, wie auch schon von Klöppel et al. (2019) \cite{kloeppel_performance_2019} als Kritik eingebracht, wurden zwar bereits viele Untersuchungen zur Auswirkung von Traffic Light Assistance Services auf Verkehrsteilnehmer durchgeführt. Viele Ergebnisse zum Impact sind aber vermutlich sehr optimistisch, da sie die Komplexität eines Tests bei alltäglichen Fahrten in der echten Welt unterschätzen. Da es deutlich weniger Fahrradstudien als Tests an Autos oder anderen größeren motorisierten Fahrzeugen gibt, ist das Forschungsdefizit hier noch viel deutlicher -- und die Impacts einer GLOSA auf Radfahrer in einer alltäglichen stadtweiten Nutzung noch weitestgehend ungeklärt.
