...
Based on the different speeds during the approach, we can calculate the increased or decreased energy spending. 
\begin{equation}
\text{Power} = (F_{\text{air}} + F_{\text{roll}} + F_{\text{climb}}) \times \text{speed}
\end{equation}

Where:
\begin{equation}
\begin{align*}
F_{\text{air}} &= \frac{1}{2} \rho \cdot v^2 \cdot C_w \cdot A \\
F_{\text{roll}} &= m \cdot g \cdot C_r \\
F_{\text{climb}} &= m \cdot g \cdot \sin(\arctan(\text{inclination}))
\end{align*}
\end{equation}
