...
Some criticism is also directly related to the user interface. In 5 cases (14\%), users were generally confused by some user interface elements. For example, one user initially interpreted the traffic light countdown as a recommended target speed. 7 users (19\%) desired a different kind of integration, such as a bike computer or an auditive speed advisory that can be used from the pocket. Further criticism regarded the speedometer's design and placement (3), such as speed ticks on the speedometer's border that were perceived to be missing. 

General criticism on speedometer design or placement: 3 (8%)
Battery usage too high: 4 (11%)
Address search should be optimized: 3 (8%)
Too much app content: 2 (6%)
Occasional app crashes: 2 (6%)
User interface not as responsive as desired: 1 (3%)

5 users (14\%) also reported occasional mismatched traffic lights. However, mismatchings were only noted and not considered as a critical issue.
Positioning experienced as inaccurate or infrequently updated: 3 (8%)
Delay in rerouting too long: 3 (8%)
Limited diversity and interpretability of routing profiles: 3 (8%)
Route planning not intuitive / easy enough: 4 (11%)
More routing functionalities desired: 1 (3%)
Routes not as wished: 4 (11%)
Would have desired routing on road instead of bike path: 1 (3%)

7 users (19\%) desired a different kind of integration, for example, in a bike computer. Among these users, a multimodal speed advisory using auditory inputs was also repeatedly desired. 
