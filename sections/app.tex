...
Diverse user interfaces have been proposed over time to present a traffic light speed advisory. After a preliminary study by Iglesias et al. (2008) \cite{iglesias_i2v_2008} that presented a perception-enhancing system mirroring the traffic light's current state onto a device, Otto et al. (2010) \cite{otto_operating_2010} were the first to propose a speed-advising user interface. In their work, three variants of a mobile application were depicted: a recommended target speed, a countdown for multiple parallel lanes at the upcoming intersection, 
