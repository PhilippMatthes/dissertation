...
% Bike-specific points of interest such as air pumps, bike parking spots, repair shops, and rental stations are added through various open datasets from the city of Hamburg. The geodata is processed on a nightly basis and distributed to the app as JSON over a web proxy in our deployment. Bike accident hotspots are also extracted from the Unfallatlas\footnote{\url{https://unfallatlas.statistikportal.de/}} dataset, clustered, and can be displayed in the mobile application.
...
Additional services: bike route details, traffic flow prediction
