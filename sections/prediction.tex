...
- Es gibt ebenfalls Punkte, die bei einer niedrigen cycle-indepdendent instability liegen, aber eine hohe cycle-dependent instability aufweisen. Dies deutet auf die Existenz von Stufenmustern hin, kann aber auch aus Datenfehlern wie fehlenden Zyklus-Observations resultieren
...
- \Cref{fig:predictability-heatmap-hourly} schlüsselt den Zusammenhang zwischen cycle-dependent und cycle-independent instability nochmals im Tagesverlauf auf. Vergleicht man die Stunden, so sieht man einen klaren Unterschied zwischen vor 6 Uhr und nach 6 Uhr, der bis ca. 23 Uhr anhält. Das ist auch der Moment, in dem nach \Cref{fig:adaptiveness-weekdays-distance} die Adaptivität wieder abnimmt.
- Auch hier sieht man wieder den großen Anteil Ampeln, die eine hohe Regelmäßigkeit und scheinbar eine geringe Verkehrsadaptivität zeigen.
- Speziell der Bereich, in dem stufenartige Muster zu erwarten sind, verändert sich im Tagesverlauf. Eine mögliche Ursache könnte sein, dass verkehrsabhängige Ampeln durchgängig ausgelöst werden und somit relativ regelmäßig schalten. Klar erkennbar wird dies aus dieser Darstellung aber nicht. 
