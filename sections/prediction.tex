...
To gain a better understanding of the implications of the presented measurements on predictability, let's examine a few randomly selected concrete case studies. Figure \ref{fig:types-of-instability} illustrates four such cases, depending on their location in the predictability heatmap shown in Figure \ref{fig:predictability-heatmap}. We focus on the remaining window after error correction, up to a cycle-dependent instability of 25 seconds.

Among the examples, both expected and unexpected patterns emerge. Case C is particularly interesting, revealing step-like patterns. These patterns often indicate a fully traffic-dependent signal that is triggered very frequently, occasionally resulting in longer waiting times. However, based on the random sample, it can be inferred that such patterns are generally rare. Overall, most patterns, as described by Protschky et al. (2014) \cite{protschky_extensive_2014}, appear to be synchronized with the cycle length in this range.

In the C category of the sample, traffic lights frequently switch between two behaviors. Although the switching pattern within each behavior is stable and aligned with the cycle, the median of cycle-dependent instability is high. Consequently, there are only a few different waiting times between green phases. Programs located in the lower right of the diagram are relatively well predictable, at least for the secure recurring green phase, even with the probabilistic approach. Step patterns seem to be less prevalent, even for traffic lights measured in the lower right of the diagram.

In the D category in the upper right, as expected, there are initially traffic lights with strong adaptability. Surprisingly, this area also contains traffic lights that occasionally switch at a recurring time. More frequently, however, there are traffic lights with data errors. Blue lines indicate cycles that are discarded more often due to detected faulty switching behavior. Specifically, cycle observations are frequently missing, resulting in the cycle length doubling or tripling. Overall, it can be observed that many traffic lights measured in the upper right have a high data error rate. Assuming perfect data transmission, the predictability of these traffic lights is likely to be relatively poor.
