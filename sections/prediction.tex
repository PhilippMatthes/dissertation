...
\begin{equation} \text{Shift}(L_1, L_2) = [g_{start}(L_2) - g_{start}(L_1), g_{end}(L_2) - g_{end}(L_1)] \end{equation}
...
\begin{equation} \text{Distance}(L_1, L_2) =  \sum_{i=0}^{\max(l_1, l_2)-1} \left\{
\begin{array}{ll}
...
- Bisherige Arbeiten haben sich nur eingeschränkt damit beschäftigt, was Prognostizierbarkeit von Ampeln eigentlich genau bedeutet. Genaue Kriterien oder Metriken, nach denen entschieden werden kann, welches Verfahren das geeignetste ist, gibt es bisher nicht. In diesem Abschnitt werden wir solche Kriterien ableiten und ein systematisches Modell erarbeiten, mit dem wir herausfinden können, wie gut unser Prognosemodell geeignet ist.
- Mit den zur Verfügung stehenden Echtzeitdaten der Ampeln in Hamburg können wir unser Vorgehensmodell anschließend umsetzen und Erkenntnisse über die tatsächliche Prognostizierbarkeit von Ampeln ableiten.

- Wie gut prognostizierbar eine Ampel ist, hängt davon ab, wie gut sich Muster in der Schalthistorie erkennen lassen. 
- Der einfachste Fall ist, wenn eine Ampel immer in gleichbleibenden zeitlichen Abständen zwischen Grün und Rot schaltet.
- Mit einer zunehmenden Adaption des Programms an den Verkehr verschieben sich prognostizierbare Muster, da Grünphasen früher oder später beginnen als gewohnt. Je zufälliger diese Adaption ist, desto schwieriger ist eine Prognose des Programms.
- Wir müssen also eine Möglichkeit finden, die Abweichung der Ampelschaltung von einem normalen Muster, also die Instabilität zu messen.
...
\hline
\rotatebox{90}{\footnotesize{\textbf{High cycle-independent instability}}} & B \includegraphics[width=0.42\linewidth]{images/predictability-cycles-1.pdf} & D \includegraphics[width=0.42\linewidth]{images/predictability-cycles-2.pdf} \\
\hline
\rotatebox{90}{\footnotesize{\textbf{Low cycle-independent instability}}} & A \includegraphics[width=0.42\linewidth]{images/predictability-cycles-3.pdf} & C \includegraphics[width=0.42\linewidth]{images/predictability-cycles-4.pdf} \\
\hline
\end{tabular}
\caption{.}
...
- Die Instabilität lässt sich zyklusabhängig oder von Grünphase zu Grünphase (zyklusunabhängig) messen. Warum eine differenzierung in diese beiden Klassen wichtig ist, sehen wir in \Cref{fig:types-of-instability}. 
- A: Jeder Zyklus schaltet gleich, mit zwischenzeitlich eventuell auftretendem Programmwechsel. Solche Programme sind am stabilsten und daher auch am besten prognostizierbar.
- B: Die Grünphase wird nur gelegentlich freigeschaltet, dann aber zu einem festen Zeitpunkt im Zyklus. Schwerer zu prognostizieren ist der Zeitpunkt der Freischaltung, während das auftretende Muster im Zyklus sehr gleichmäßig ist. Die zyklusunabhängige Instabilität ist groß, während die zyklusabhängige Instabilität gering ist.
- C: Rechts unten wird die Grünphase sehr regelmäßig freigeschaltet, aber verschiebt sich mit jeder Freischaltung im Zyklus. Die zyklusabhängige Instabilität ist groß, während die zyklusunabhängige Instabilität gering ist.
- D: Rechts oben sind beide Arten von Instabilität sehr hoch. Die Prognostizierbarkeit ist somit stark eingeschränkt, für alle Arten von Modellen.

- Mithilfe dieser Unterteilung können wir uns überlegen, welche Prognoseverfahren im jeweiligen Fall geeignet wären:
- Im einfachsten Fall A liegt der Fokus auf der Skalierbarkeit und Robustheit des Prognoseverfahrens gegenüber fehlenden Daten.
- In Fall B wäre ein probabilistisches Verfahren anstelle eines selbstadaptiven Verfahrens zu präferieren da es die Chance in manchen Fällen eine Freigabe zu erhalten trotzdem reflektiert und somit die Möglichkeit bietet, sich an den Bereich, in dem eine Grünphase geschaltet werden könnte, zu adaptieren.
- Fall C ist sehr schlecht vom probabilistischen Modell prognostizierbar. Eine gute Alternative für diese Art von Schaltverhalten sind graphbasierte Methoden, da diese die Phasenlänge statt der Lokalität im Zyklusvektor zur Prognose nutzen.
- Bei Fall D lohnt sich die Überlegung, ein rechnerisch aufwändigeres Machine Learning Modell zu nutzen, da diese die höchste Chance haben, komplexere Muster zu erkennen.
- Eine Messung der zyklusabhängigen und zyklusunabhängigen Instabilität gibt uns also einen guten Richtwert zur Evaluation, wie gut prognostizierbar Ampeln sind und wie sinnvoll unser gewähltes Verfahren auf die Situation vor Ort passt.


\begin{lstlisting}[language=Python, caption=Python implementation of the cycle-dependent instability metric.]
...
\begin{lstlisting}[language=Python, caption=Python implementation of the cycle-independent instability metric.]
...
Was noch fehlt
- Betrachtung der Grünphasenlänge/Grünphasenüberschneidung und Shift
- Diskussion der Ergebnisse: speziell auch näher auf immer rote Ampeln eingehen
- Was bedeuten die Ergebnisse für das Prognoseverfahren und andere Prognoseverfahren?
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{images/predictability-cycle-independent.pdf}
    \caption{.}\label{fig:}
\end{figure}
