...
Both commercial providers, Bing Maps and Google Maps, seem to produce a less detailed geometry in the given example. Minor bends are not captured. Finally, there are also visible detours with Google Maps at the start of test route \texttt{1\_Ost\_West} and the Walking mode of Bing Maps at the end of \texttt{3_West_Ost} that we will investigate later.

As presented in \Cref{tab:accuracy-comparison}, the observations from this one case study also align with the general measurements on all six reference routes. 
