...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{8cm}p{16cm}}
        \hline
        \textbf{Tag in DRN} & \textbf{Mapped Tag in OpenStreetMap} \\
        \hline
        \textit{Aufgeweiteter Radaufstellstreifen} & \texttt{highway: tertiary} \\
        \textit{Busfahrstreifen mit Radverkehr} & \texttt{cycleway: share\_busway, highway: service} \\
        \textit{Fähre} & \texttt{route: ferry, bicycle: yes} \\
        \textit{Fahrradstraße} & \texttt{bicycle\_road: yes, bicycle: designated, maxspeed: 30, source:maxspeed: DE:bicycle\_road, traffic\_sign: DE:244.1, highway: residential} \\
        \textit{Fußgängerüberweg/-furt (Schiebestrecke)} & \texttt{highway: pedestrian} \\
        \textit{Fußgängerzone - immer befahrbar} & \texttt{highway: pedestrian, bicycle: yes} \\
        \textit{Fußgängerzone (meist zeitlich begrenzt)} & \texttt{highway: pedestrian} \\
        \textit{Fußgängerzone - zeitlich begrenzt} & \texttt{highway: pedestrian} \\
        \textit{Gehweg (Fahrrad frei)} & \texttt{highway: footway, foot: designated, bicycle: yes, traffic\_sign: DE:239,1022-10} \\
        \textit{Gehweg (nur ZZ 1022-10)} & \texttt{highway: footway, foot: designated, bicycle: yes, traffic\_sign: DE:239,1022-10} \\
        \textit{Gehweg (Schiebestrecke)} & \texttt{highway: footway} \\
        \textit{Gemeinsamer Geh-/Radweg} & \texttt{highway: path, bicycle: designated, foot: designated, segregated: no} \\
        \textit{Getrennter Geh-/Radweg} & \texttt{highway: path, bicycle: designated, foot: designated, segregated: yes} \\
        \textit{Kommunaltrasse} & \texttt{cycleway: share\_busway, highway: tertiary} \\
        \textit{Kopenhagener Radweg} & \texttt{highway: cycleway, bicycle: designated} \\
        \textit{Protected Bike Lane} & \texttt{highway: cycleway, bicycle: designated} \\
