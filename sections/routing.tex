...
Establishing an accurate bike routing is a multifaceted research area.

- Eine Forschungsrichtung beschäftigt sich mit Modellen, die genutzt werden können, um die Routenwahl von Radfahrern möglichst genau anhand der verfügbaren Metadaten vorherzusagen. Hierfür lassen sich GNSS-Trajektorien mit Routing-Netzwerken korrelieren, um Zusammenhänge zwischen den Wegeigenschaften und der Routenwahl nachzuvollziehen. Es gibt beispielsweise auch schon Studien, welche die kurzfristigen Effekte von Infrastrukturveränderungen auf die Routenwahl von Radfahrern (Oslo) messen. Über die Jahre haben sich auch Routing-Engines immer weiter verbessert, wodurch mittlerweile sehr durchgängig getestete Routingprofile für unterschiedliche Radfahrtypen zur Verfügung stehen. Für den Anwendungsfall eines Fahrradroutings in einer Bike-GLOSA app können die existierenden Routingprofile als genau genug eingeschätzt werden. 

- Was jedoch noch nicht als genau genug eingeschätz werden kann, ist das Alignment der generierten Routen mit der tatsächlichen Position der Radwege. Wir werden uns ansehen, welche Methoden und Ideen vorliegen um dieses Problem zu adressieren. 
- Anschließend werden wir uns einige bisher eher theoretische Konzepte ansehen, für die Routing eine Schlüsseltechnologie darstellt, um diese in die Praxis umzusetzen: Multi-segment GLOSA, GNSS Fehlerkorrektur, Kontextsensitive speed advisories, traffic light matching, und distance-to-signal estimation.
...
In their route-based approach to traffic light matching, Mahler et al. (2012) \cite{mahler_reducing_2012} propose Google Maps as a routing service, although we must note that Google Maps does not necessarily exhibit superior qualities to OpenStreetMap. Ciepluch's comparison in 2010 \cite{ciepluch_comparison_2010} revealed that Google Maps, Bing Maps, and OpenStreetMap did not significantly differ in quality, with no platform emerging as a clear winner. Similarly, a more recent study by Franzini et al. (2020) \cite{franzini_assessment_2020} found that only 40\% of the Pavia region in Italy is fully mapped in OpenStreetMap and 30\% in Google Maps, based on 2018 data. These findings emphasize the incomplete coverage of both platforms, though this study is not specifically focused on bicycle paths.
