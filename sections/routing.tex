...
- Ob sich diese Verbesserung auch, wie erwartet, positiv auf die Matching-Genauigkeit auswirkt, sehen wir im nächsten Experiment. Hierfür trainieren wir zunächst das algorithmische Matching-Modell erneut auf der neu erstellten Ground truth.
- Die gefundene Hyperparameter-Konfiguration mittels Hyperparameter-Optimierung ist in \Cref{tab:hyperparameter-tuning-results-drn} denen der OpenStreetMap-Modelle gegenübergestellt.
- Die Thresholds wurden vom Optimierungs-Tool etwas strikter gewählt als zuvor in OpenStreetMap. Striktere Thresholds bedeuten, dass traffic light geometries schneller verworfen werden, wenn sie nicht gut zur Route passen.
...
- Auch das Machine-Learning modell aus wird erneut wie in \Cref{ch:matching} mit stratified k-fold cross validation auf der DRN-Ground Truth trainiert.
- Als Ergebnis erreicht das machine learning modell während des Trainings einen F1-Score von 95.83\% und einen Test Score von 90.94\%. Während sich der Training Score also um fast 2 Prozentpunkte verbessert hat, liegt der Test score ca. 1\% unter dem Score auf OpenStreetMap. Eine Vergleichbarkeit zwischen den unterschiedlichen Splits herzustellen ist aber nicht ohne weiteres möglich. Der Test Score des OpenStreetMap modells kann auch dadurch höher gewesen sein, dass der Datensatz ca. drei mal so viele Routen beinhaltete, und nur ca. halb so viele traffic light geometries. 

- \Cref{tab:model-scores-drn} zeigt ein gegenteiliges Bild: Über den gesamten Datensatz evaluiert liegt der F1-Score bei 94.0\% und damit ca. 2\% höher als auf den OpenStreetMap Daten. Der Recall konnte auf 97\% verbessert werden, es sind also weniger falsch-positive festzustellen.
- Auch das algorithmische Modell konnte sich verbessern. Während das algorithmische Modell auf dem OpenStreetMap Datensatz aus 2022 sein bestes Ergebnis von 84.8\% erzielte, liegt das neu trainierte Modell auf DRN 3.2\% darüber bei 88.0\%.

- Insgesamt manifestiert sich also die messbare Verbesserung in der Genauigkeit des Fahrradroutings nicht nur in der stärkeren Striktheit des algorithmischen Modells, sondern auch generell in einer verbesserten Matching-Genauigkeit. Der meiste Fortschritt wurde allerdings bei dem weniger genauen algorithmischen Modell verzeichnet.
- Da das algorithmische Modell mehr Genauigkeit hinzugewonnen hat als das Machine Learning modell, könnte man mutmaßen, ob dies vielleicht mit einer generellen Vereinfachung des Matching-Problems zusammenhängt. Daher lohnt sich hier ein Blick auf die Arten der Konstellationen zwischen Routen und traffic light geometries, die mit unterschiedlich schwierigen Entscheidungen für das Matching-Modell verbunden sind.
...
- Wie \Cref{fig:matching-constellations-osm-vs-drn} zeigt gibt es in jedem Fall fast doppelt so viele Beispiele, bei denen die Routengeometrie fast identisch mit der traffic light geometry verläuft.
- \Cref{fig:matching-constellations-osm-vs-drn} zeigt auch, dass deutlich seltener Routen versetzt zur Ampelgeometrie verlaufen. Dieser Fall wurde in OpenStreetMap vorallem dann beobachtet, wenn die Route auf die Straße führt obwohl ein Fahrradweg zur Verfügung steht. Die Fehlerfälle beim Matching wurden hier fast komplett eliminiert.
- Ein ähnlicher Unterschied ist bei Situationen zu sehen, in denen die Route abbiegt, aber versetzt zur Route läuft. Dies passiert weniger häufig mit DRN, ist aber im Verhältnis zur Häufigkeit nach wie vor die Situation mit dem höchsten Fehlerpotenzial. Das ML-Modell handhabt diese Situation auch im DRN-Datensatz besser als das algorithmische Modell.
- Mit dem neuen Routing ist aber auch eine neue Fehlersituation entstanden, speziell wenn die Route und die traffic light geometry fast identisch verlaufen. Hier gibt es also unter Umständen noch Verbesserungspotenzial für das Matching-Verfahren
...
- Auch mit Blick auf Routing-Fehler, die bei der Erstellung der Ground Truth unterteilt in vier Klassen mit angegeben wurden, hat sich eine deutliche Verbesserung ergeben.
- Die Anzahl der Situationen ohne Routing fehler wurde fast verdreifacht, von 23\% in OpenStreetMap auf 64\% aller Samples in DRN. Der Hauptgrund für die Verbesserung ist, dass nun die Fahrradfurten und Fahrradwege viel häufiger korrekt genutzt werden, anstelle in der Mitte der Straße zu routen. 
- Vollständig eliminiert werden konnte dieser Fall jedoch nicht. Außerdem wird deutlich seltener auf der falschen Straßenseite geroutet.
- Eine detailliertere Fallstudie, in der Max Lorenz 72 zufällige Routen untersucht hat, kommt zu ähnlichen Ergebnissen. Diese sind in \Cref{tab:accuracy-comparison} gezeigt.
- Auch in dieser Validierungsstudie zeigt sich, dass die Routingqualität deutlich verbessert werden konnte, indem deutlich weniger Fehler identifiziert werden konnten. Der größte Unterschied ergibt sich auch hier durch das kontinuierlichere Routing auf dem Fahrradweg, aber auch andere, seltenere Fehlerfälle wie das Routing durch nicht passierbare Hindernisse, Gebäude und Treppen konnten mitigiert werden. 
...
- Insgesamt zeigt sich also nicht nur, dass die DRN Routen deutlich näher auf der Fahrradinfrastruktur liegen. Auch das Alignment mit den Ampelgeometrien konnte deutlich gegenüber OpenStreetMap-Fahrradrouten verbessert werden. Die Ergebnisse suggerieren, dass dies auch das Traffic Light Matching begünstigt. Die Routen Fehlerrate an Kreuzungen konnte substantiell verringert werden. Auch bei Untersuchungen ganzer Routengeometrien fallen weniger Fehler auf.

\subsection{Alignment with User Trajectories}

- Abschließend betrachten wir noch, wie gut das DRN Netzwerk im Vergleich mit den Fahrten der Nutzer übereinstimmt. Außerdem schauen wir uns an, wie gut die routenbasiert abgeschätzte Distanz zur Ampel im Vergleich mit der tatsächlichen Distanz die zur Ampel zurückgelegt werden wird übereinstimmt, im Vergleich mit der direkten Luftlinie.
...
- Indem wir die GNSS-Trajektorien der Nutzer über beide Routennetzwerke legen, können wir sichtbar machen, ob die tatsächlichen Positionen eher mit dem OpenStreetMap-Netzwerk übereinstimmen oder mit dem DRN-Netzwerk. Zu beachten ist hierbei, dass die GNSS-Position natürlich um die tatsächliche Position streut. In der Masse der Tracks sollten wir aber eine Tendenz sichtbar machen können, unter der Annahme, dass das GNSS annähernd normalverteilt um die tatsächliche Position streut und Effekte wie GNSS Drift, die diese Streuung verschieben, vernachlässigbar sind.
- Das Ergebnis ist in \Cref{fig:routing-mapmatching-distance} geplottet. Es handelt sich hierbei um zwei Kreuzungen aus dem Zentrumsbereich Hamburgs, die gewählt wurden, um Wohnorte oder ähnliche erkennbare personenbeziehbare Rückschlüsse in den Tracks weitestmöglich zu verhindern
- Trotz der sichtbaren Streuung des GNSS lässt sich erkennen, dass die aufgezeichneten Radfahrtrajektorien deutlich besser mit dem DRN-Pfadnetzwerk übereinstimmen als mit OpenStreetMap.
...
- Da dies natürlich nur einen Teil der Pfade visualisiert, führen wir noch einen aggregierten Vergleich aller Trajektorien mit dem Routennetzwerk durch. Dafür map-matchen wir die aufgezeichneten Tracks über die GraphHopper mapmatch API\footnote{\url{https://docs.graphhopper.com/\#tag/Map-Matching-API}} welche das Verfahren von Newson and Krumm \cite{newson_hidden_2009} nutzt auf OpenStreetMap und DRN. Anschließend messen wir die Distanz von jedem GNSS-Punkt über den Vincenty-Estimator zum nächstgelegenen gemapmatchten Segment.
- Als Ergebnis sehen wir auch hier wieder zwei schöne halbseitige Normalverteilungen, wobei die Positionen bei DRN im Median 2.90m vom nächsten Pfad und in OpenStreetMap 4.41m entfernt sind. Die Streuung ist in DRN auch deutlich geringer mit einem IQR von 5.40m vs. 7.09m in OpenStreetMap. Auch hier macht sich also die genauere und konsistentere Abbildung der Fahrradwege bemerkbar.
- Für die GNSS-Korrektur, die über das Snapping auf die Route fungiert, bedeutet das insgesamt, dass die tatsächliche Position des Nutzers, auch zur Darstellung im User Interface, deutlich weniger verfälscht wird.
...
\caption{.}
\label{fig:routing-distance-analysis}
\end{figure}
