...
The DRN dataset is an authoritative routing network of the city of Hamburg, which is specially designed for cycle paths. It is published by the Free and Hanseatic City of Hamburg, Authority for Transport and Mobility Transition (BVM) and is available under the data license Deutschland Namensnennung 2.0. Under the information on periodicity, it is stated that the data set is continuously updated and is currently (as of December 27, 2023) in production. The route segments can be obtained free of charge via a public interface. The data set is marked as routable.
This means that the road segments are recorded in a continuous graph.This means that roads designated for use by cyclists are also recorded if no cycle paths are available.
