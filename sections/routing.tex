...
Whether the goal of a more accurate bike routing could be reached will be evaluated in three steps. First, we will investigate the general structure of the processed routing network and compare it to OpenStreetMap. Specifically, we will focus on the path categories that were mapped using our metadata mapping from the DRN format and compare these to the OpenStreetMap network. Afterward, a ground truth -- hand-drawn reference tracks -- will help us evaluate how well DRN routes align with actual cycling infrastructure. Here, we will also compare DRN with Google Maps and Bing Maps. Analyzing an arbitrary sample of DRN routes, we will investigate how many routing errors are encountered.
...
\caption{Mapping of path and surface types.}
...
The structural differences are highlighted in \Cref{fig:routing-drn-osm-map}. Here, it can not only be seen that OpenStreetMap provides more accessory paths, but also its remarkably higher diversity in path categories than present in the mapped DRN network. For example, the Bundesstraße 4 marked as a primary connection in OpenStreetMap is omitted in DRN, which captures the footpaths at both roadsides that are marked for bicycle usage. It can also be seen that DRN tends not to capture ways through green spaces. As a result, one minor drawback is that routes with DRN can be generated until the border of a park area, but not inside. The same is true for intermodal connections such as railroads and ferries, which are mapped as unclassified paths in the displayed example.
...
On the other hand, as seen in \Cref{fig:routing-drn-osm-map-surfaces}, OpenStreetMap contains large proportions of paths with an unclassified surface. Overall, only 66.4\% (7,014.52 km) of paths in OpenStreetMap's representation of Hamburg are tagged with a surface, as opposed to 99.9\% (7,682.24 km) in the DRN network. However, OpenStreetMap also contains an additional surface quality tag for 783.56 km (7.4\%), which is not mapped by our approach but can be parsed by GraphHopper as an additional source of information to estimate the path speed\footnote{\url{https://github.com/graphhopper/graphhopper/blob/71d2de049f2f4f52453a6fb1b7d558b9f0fa011f/core/src/main/java/com/graphhopper/routing/util/parsers/BikeCommonAverageSpeedParser.java\#L103}}. Other metadata, such as road width, is even rarer in OpenStreetMap, with only 9.2\% (704.47 km) of paths containing this information, in contrast to 99.9\% (7,687.69 km) in the DRN routing foundation. An additional property analyzed by Max Lorenz is the coverage of velo routes, which resides at 1,794.55 km in OpenStreetMap and 1,149.8 km in DRN. 
...
\toprule
\textbf{Ground}& \multicolumn{3}{c}{\textbf{DRN}}&\multicolumn{3}{c}{\textbf{OpenStreetMap}}&\multicolumn{3}{c}{\textbf{Bing Maps (Driving)}}&\multicolumn{3}{c}{\textbf{Bing Maps (Walking)}}&\multicolumn{3}{c}{\textbf{Google Maps}}\\
\textbf{Truth}& \textbf{HD} & \textbf{Mean} & \textbf{W} & \textbf{HD} & \textbf{Mean} & \textbf{W} & \textbf{HD} & \textbf{Mean} & \textbf{W} & \textbf{HD} & \textbf{Mean} & \textbf{W} & \textbf{HD} & \textbf{Mean} & \textbf{W} \\
\midrule
1\_Ost\_West & 6.89 & 2.20 & 2 & 3.48 & 1.26 & 8 & 16.34 & 9.66 & 4 & 16.02 & 9.63 & 8 & 216.44 & 66.54 & 2 \\
1\_West\_Ost & 2.80 & 1.20 & 2 & 9.05 & 5.99 & 2 & 14.75 & 9.02 & 7 & 14.75 & 9.02 & 7 & 8.67 & 6.09 & 9 \\
2\_Ost\_West & 3.49 & 1.12 & 3 & 14.43 & 4.45 & 9 & 17.75 & 10.47 & 2 & 17.75 & 10.87 & 8 & 19.29 & 10.71 & 3 \\
2\_West\_Ost & 3.67 & 1.05 & 3 & 10.62 & 2.90 & 7 & 26.93 & 11.36 & 2 & 67.44 & 16.78 & 2 & 26.28 & 10.46 & 2 \\
3\_West\_Ost & 4.70 & 1.72 & 4 & 15.24 & 6.28 & 4 & 25.94 & 10.03 & 8 & 27.10 & 10.10 & 3 & 52.55 & 11.41 & 9 \\
3\_Ost\_West & 7.83 & 1.43 & 3 & 15.97 & 4.75 & 8 & 18.13 & 8.47 & 8 & 41.57 & 13.38 & 6 & 16.72 & 7.64 & 3 \\
\bottomrule
\multicolumn{16}{p{20cm}}{Abbreviations: Hausdorff distance measured in EPSG:32633 (HD), Mean Vincenty distance in meters (Mean), Number of waypoints for best convergence with ground truth (W).}\\
\end{tabular}
}
...
\begin{figure}[htbp]
\centering 
\includegraphics[width=\linewidth]{images/routing-convergence.pdf}
\caption{.}
\label{fig:}
\end{figure}
