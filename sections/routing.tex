...
Der DRN datensatz ist ein authoritatives Routingnnetzwerk der Stadt Hamburg, welches speziell für Fahrradwege ausgelegt ist. Es wird von der Freie und Hansestadt Hamburg, Behörde für Verkehr und Mobilitätswende, (BVM) veröffentlicht und steht unter der Datenlizenz Deutschland Namensnennung 2.0 zur Verfügung. Unter den Angaben zur Periodizität ist angegeben, dass der Datensatz kontinuierlich aktualisiert wird und sich derzeit (Stand 27. Dezember 2023) in Produktion befindet. Die Wegsegmente können über eine öffentliche Schnittstelle kostenlos bezogen werden. Der Datensatz ist als routingfähig gekennzeichnet. Die Wegsegmente werden also in einem zusammenhängenden Graphen erfasst. Es werden also auch Straßen erfasst, die zur Nutzung durch Radfahrer ausgewiesen sind, falls keine Fahrradwege zur Verfügung stehen. 
...
- DRN bringt also generell bereits eine sehr günstige Attribute mit, die ein Routing auf dem Datensatz erleichtern. Dadurch, dass der Datensatz die gesamte Fläche von Hamburg erfasst und nicht nur Teilbereiche, können wir die Wegsegmente von OpenStreetMap in den abgedeckten Bereichen verwerfen.
- Dennoch gibt es einige Unterschiede, die eine direkte Verbindung der beiden Datensätze verhindern. Diese Unterschiede müssen wir überbrücken.
- Der erste Unterschied ist die Sprache, in der DRN vorliegt. Die vielen eigens definierten Pfadkategorien von DRN müssen sinnvoll auf das OpenStreetMap Format abgebildet werden.
- Auch die Geometrie des DRN ist anders gestaltet als die von OpenStreetMap. Wie in \Cref{fig:comparison} gezeigt, liegen Radwege konsequent als separate Geometrien vor, sind tendenziell besser mit Ampelgeometrien aligned, und es fehlen generell manche Nebenwege oder Straßen, die nicht von Radfahrern genutzt werden sollen. 
- Die geometrische Diskrepanz in der Grenzregion zwischen DRN und OpenStreetMap werden wir mit einem Stitching-Verfahren lösen. Es gibt aber, wie wir in den nachfolgenden Sektionen sehen werden, auch andere Herausforderungen mit der geometrischen Struktur, die Lösungen erfordern.
- Wenn wir die Metadaten und die geometrische Struktur harmonisiert haben, erhalten wir unsere finale Routing-Foundation, die innerhalb von Hamburg auf dem DRN Datensatz routet, und außerhalb auf OpenStreetMap.
...
DRN speaks CSV, GeoJSON, or GML. Each contained data point refers to a path segment and is associated with metadata tags. Among others, the following properties are mapped\footnote{The displayed statistics represent a snapshot of the 27th of December 2023.}:
