...
\caption{Duplicated nodes (left) are merged together (right) to avoid detours. [\hyperref[attribution]{Attribution}]}
...
\caption{Detour (left) that can be fixed by enabling opposite-direction one-way travel on foot (right), compared to OpenStreetMap route (blue). [\hyperref[attribution]{Attribution}]}
...
\caption{Constellations between traffic light geometries and routes in DRN relative to the OpenStreetMap results.}
...
\caption{Routing errors tagged for each constellation between traffic light and route, compared between OpenStreetMap and DRN.}
\label{fig:matching-route-errors-osm-vs-drn}
\end{figure}

As depicted in \Cref{fig:matching-route-errors-osm-vs-drn}, an examination of routing errors, categorized into four classes during ground truth creation, reveals a significant enhancement. Instances without routing errors nearly tripled, rising from 23\% (241 out of 1035 constellations marked) in OpenStreetMap to 60\% (438 out of 731 constellations marked) in DRN. This improvement primarily stems from more accurate utilization of bike lanes and paths, reducing routing in the middle of the road. However, complete elimination of this scenario was not achieved. Moreover, routing on the wrong side of the street occurs less frequently.
...
\hline
\end{tabular}
\caption{Observed error cases among an arbitrary sample of 72 routes.}%
\label{tab:accuracy-comparison}%
\end{table}
