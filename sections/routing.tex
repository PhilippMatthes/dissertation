...
- Unter den Angaben zur Periodizität ist angegeben, dass der Datensatz kontinuierlich aktualisiert wird und sich derzeit (Stand 27. Dezember 2023) in Produktion befindet
- Die Wegsegmente können über eine öffentliche GML-Schnittstelle, aber auch in verschiedenen anderen Formaten (GeoJSON, CSV) kostenlos bezogen werden. Die Geometriedaten liegen im Koordinatensystem EPSG:25832\footnote{\url{https://epsg.io/25832}} vor.
- Der Datensatz ist als routingfähig gekennzeichnet. Die Wegsegmente werden in einem zusammenhängenden Graphen erfasst. Es werden also auch Straßen erfasst, die zur Nutzung durch Radfahrer ausgewiesen sind, falls keine Fahrradwege zur Verfügung stehen. 
...
- DRN unterscheidet sich auf vielen verschiedenen Ebenen von OpenStreetMap. Einer der wichtigsten Unterschiede ist die geometrische Struktur des DRN, die auch in \Cref{fig:comparison} gezeigt ist.
- Die Pfadgeometrien überschneiden sich mehr mit den Ampelgeometrien, scheinen also auf den ersten Blick besser zur Radinfrastruktur zu passen. Wege in der Mitte stark befahrener Straßen fehlen, dafür sind die Radwege scheinbar 
