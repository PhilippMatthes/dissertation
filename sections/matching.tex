...
For cyclists, the situation is quite different. As initially described, the issue for cyclists is that many traffic lights can be approached from multiple directions. The result is visible in \Cref{fig:ingress-lane-lengths}, with ingress lane lengths, if all bicycle lanes marked in green in the diagram are considered together, having a median length of 1 meter. In many cases, ingress lanes would need to be artificially extended in multiple directions, or at least multiple longer ingress lanes would need to be provided. This would require a restructuring of all intersection topologies.
...
However, the developed approach also has limitations that open up new opportunities for future work. An alternative solution to the proposed matching algorithms is arriving at a georeferenced bicycle network graph that contains all traffic light IDs. Such a network graph is currently not available for Hamburg \cite{neuner_leitfaden_2020} but could be developed in the future. The accuracy of bike path in proprietary HD maps could not be investigated in this study due to the limited availability of these datasets.

- Würde man zu einem georeferenzierten Netzgraphen kommen, könnte man auf dem Netzgraphen die Fahrradroute errechnen und zugleich alle Ampeln determinieren. In diesem Fall ist die Anwendung eines Matchings wie in dieser Arbeit gezeigt also nicht notwendig. In Hamburg ist ein solcher maintainter Netzgraph, auch für Autos, aber nicht verfügbar \cite{neuner_leitfaden_2020}. Auch bei proprietären HD-Maps stellt sich die Frage, ob diese die Fahrradwege gut abbilden. 

- Dieser Frage konnte mangels Verfügbarkeit nicht nachgegangen werden und sollte durch zukünftige Arbeiten geklärt werden. Im Idealfall wäre es sogar möglich, mit öffentlich verfügbaren Daten einen solchen Netzgraphen für Fahrradwege automatisiert zu erzeugen. Zu lernen wie man diese Idee in die Praxis umsetzt, und ob dies überhaupt möglich ist, wäre auch ein potenzieller Gegenstand zukünftiger Arbeiten
- Status quo gibt es eine auf der Hand liegende Möglichkeit, das Matching zu verbessern: die hohe Anzahl Routingfehler. Diesem Problem werden wir uns im nächsten Kapitel widmen