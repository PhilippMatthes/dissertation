...
Since two routings will be utilized (OpenStreetMap and DRN -- see \Cref{ch:routing}), coming with different alignments of the generated routes to the traffic light geometries, the model must be tuned separately on both datasets. Therefore, one ground truth is created for each routing foundation. The final model configuration for both routing foundations is highlighted in \Cref{tab:hyperparameter-tuning-results}. Here, it is visible that the DRN version of the filtering pipeline has slightly more strict thresholds than the OpenStreetMap version. This is expected as the goal of the DRN routing foundation developed in \Cref{ch:routing} is to provide a more accurate bike routing, which is supposed to align more accurately with the traffic light geometries.
...
- Stand 8. Dezember 2023 haben 677 von 791 (85.6\%) im System verfügbaren Kreuzungsknoten mindestens eine dedizierte Fahrradampel. An 194 (24.5\%) Kreuzungen gibt es mindestens eine Ampel, die sich Radfahrer und KFZ teilen können. 53 Kreuzungen (6.7\%) besitzen mindestens eine Ampel die Mischverkehr von Radfahrern, KFZ und Bussen zugeordet ist. Abschließend gibt es noch 24 Kreuzungen (3\%) die von Radfahrer und Fußgängern benutzt werden sollen. 

- Die Prozentzahlen überschneiden sich aber, da es häufig pro Kreuzung und Fahrtrichtung mehrere parallele Ampeln gibt, die theoretisch genutzt werden können, von denen aber nur eine schließlich die beste Wahl ist
...
- Bei vielen Ampeln müsste man die Einfahrtsspur künstlich in mehrere Richtungen verlängern, oder zumindest mehrere längere Einfahrtsspuren zur Verfügung stellen. Dies würde eine Umstrukturierung sämtlicher Kreuzungstopologien erfordern
- Selbst dann ist jedoch das Problem noch nicht gelöst, 

\subsection{Coverage and Representativeness of Ground Truth}

\begin{figure}[htbp]
\centering 
\includegraphics[width=\linewidth]{images/binding-coverage-hamburg.png}
\caption{Randomly generated and labeled routes in ground truth dataset.}
\label{fig:}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\linewidth]{images/coverage-analysis.pdf}
\caption{Relation between the number of lanes and the number/length of routes in the ground truth.}
\label{fig:}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\linewidth]{images/coverage-analysis-2.pdf}
\caption{Curvature and length of matching and non-matching lanes in ground truth.}
\label{fig:}
\end{figure}
...
\caption{Visualization of the ML model inference process. Checkmark: predicted to match. Cross-mark: not predicted to match.}
\label{fig:}
\end{figure}
...
\begin{figure}[htbp]
\centering 
\includegraphics[width=\linewidth]{images/matching-constellations.pdf}
\caption{Model accuracy depending on the type of lane-route constellation.}
\label{fig:}
\end{figure}
...
\begin{figure}[htbp]
\centering 
\includegraphics[width=\linewidth]{images/matching-route-errors.pdf}
\caption{Model accuracy depending on the type of routing error.}
\label{fig:}
\end{figure}
