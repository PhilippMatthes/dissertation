...
Detecting traffic lights is not only a safety-critical functionality for (semi)autonomous driving systems but is also essential for bridging the gap between traffic light prediction and speed recommendation. In \Cref{ch:prediction}, we addressed the prediction of traffic lights and developed a system that allows us to obtain predictions for a subset of traffic lights in Hamburg over the internet.

We have previously considered that each traffic light has an ID (also referred to as connection). The prediction is obtained through this ID. However, we have not yet explored how to determine which traffic lights are relevant for the user's route. To implement this functionality, we require georeferencing to identify the location of each traffic light with its corresponding ID. Subsequently, this information can be matched with the user's route.

The concept of georeferencing intersections, initially termed Geometric Intersection Description (GID), was introduced relatively early in the USA in 2008 \cite{cicas-v}. Recognizing the importance of georeferencing traffic lights for the implementation of autonomous driving systems or Cooperative Intelligent Transportation Systems (C-ITS), an international standard known as MAP-Message, MAP-Topology, or simply MAP was developed. Both MAP and Signal Phase and Timing (SPaT) are defined by the SAE J2735\footnote{\url{https://www.sae.org/standards/content/j2735_202309/}} standard. A MAP contains the lane geometries of all turns at an intersection and is usually maintained by engineering bureaus tasked with traffic development.

MAPs, like SPaTs, can reach the road user through various channels, encountering similar transmission issues as SPaTs (see \Cref{ch:prediction}). However, since MAPs are less frequently updated, collecting, transferring to a database, and redistributing the lane geometries is relatively straightforward. For instance, Hamburg provides the lane geometry associated with each traffic light (connection) publicly through the Traffic Lights Data platform.

A more significant challenge than the acquisition of intersection topologies is determining which lane on the intersection will be used by a road user approaching it. There are typically multiple individual paths that can be chosen when crossing an intersection. The choice of turn depends on the user's route and mode of transportation. While cars are restricted by available lanes, cyclists or pedestrians often have multiple options to cross an intersection. Moreover, cyclists or pedestrians may approach a traffic light from several directions, leading to no clear entry lane.

Selecting the correct traffic light for a cyclist is, therefore, not a trivial problem. The objectives of this chapter are as follows:

\begin{enumerate}
    \item Reviewing related work and how they address the problem.
    \item Identifying three core approaches: vision-based using cameras or similar sensors for traffic light identification, location-based using global navigation and satellite systems (GNSS) for lane positioning, and route-based using semantic information such as turns for lane selection.
    \item Exploring why route-based approaches are currently the most promising for bicycle applications and developing two novel approaches to implement this as accurately as possible.
    \item Discussing approaches that did not yield the desired results and the reasons behind them.
    \item Evaluating the accuracy of the developed approach using multiple ground truths to make well-founded statements about the suitability of the method.
    \item Providing a critical summary of the state of the art, developed methods, and results to indicate directions for future work.
\end{enumerate}

\section{Related Work}

The need to address traffic light matching as a problem is determined by the study environment.

Simulation studies can shortcut the traffic light matching problem through the simulation environment that knows for each agent which sequence of turns will be taken. This means that it is possible to assume that each agent is informed about the upcoming traffic light. SUMO, the most popular simulation environment among GLOSA studies \cite{krajzewicz_preparing_2012, erdmann_combining_2013, eckhoff_potentials_2013, tal_vehicular-communications-based_2016, nguyen_efficient_2016, olaverri-monreal_implementation_2018, karoui_efficiency_2018, pariota_green_2019, kloeppel_performance_2019, lu_green_2020, halbach_cooperative_2021, bhattacharyya_assessing_2022, grumert_heads-up_2022, wagner_spatmap_2023}, offers a special API for traffic light matching within the TraCI interface. This interface is utilized by Krajzewicz et al. (2011) \cite{krajzewicz_preparing_2012}, Klöppel et al. (2019) \cite{kloeppel_performance_2019}, Halbach et al. (2021) \cite{halbach_cooperative_2021}, Grumert and Pereira (2022) \cite{grumert_heads-up_2022}, Wagner et al. (2023) \cite{wagner_spatmap_2023}, and Schlamp et al. (2023) \cite{todo}, allowing the authors to find upcoming traffic lights through the simulation environment which knows about the relation between the path network and each traffic light. There are also studies assuming that only one traffic light is associated with each ingress direction at the intersection \cite{xia_indirect_2011, li_multi-vehicles_2014, plianos_predictive_2018}. In general, the easiest way of determining the traffic lights is by preselecting them in advance.

However, it is clear that these solutions are not applicable to the real world. Preselecting traffic lights may be possible in test track environments with a predefined vehicle route and only a few intersections \cite{chen_developing_2022} or corridors \cite{fickas_fast_2019}. Yet, since an approach is desired in which users are allowed to travel freely throughout the city, other methods must be identified. Furthermore, the information about traffic light positions must be associable with the traffic light prediction, for example, through a traffic light ID. This highly specialized information requires using advanced map material in correspondence with the planned vehicle trajectory over the intersection. In the following sections, we will discuss three possible solutions: vision-based, location-based, and route-based approaches.
...
An exception is the work of Koukoumidis et al. (2011–2012) \cite{koukoumidis_signalguru_2011} \cite{koukoumidis_leveraging_2012}, which employs a vision-based method suitable for smartphones. The position and relevance of traffic lights for speed recommendations are detected only using the smartphone's camera and the user's movement direction. Additionally, the color of the traffic light is detected to obtain its state directly. In a collaborative approach, information about traffic light phases is gathered as users pass by, and a prediction is generated from the sparse crowdsourced data. The smartphone is mounted on the inside of the windshield, resulting in reported accuracy ranging from 87.6\% to 92.2\%, depending on the deployment city under investigation. However, this accuracy calculation includes frames where the vehicle is stationary at a traffic light, making it challenging to compare with other studies based on per-traffic-light matching accuracy. In any way, like other vision-based approaches from the field of autonomous driving, the main limitation of this approach is its reliance on a clear line of sight. A handlebar-mounted or stowed-away smartphone, such as in the case of a bike-GLOSA app, does not provide this line of sight.
...
Katsaros et al. (2011) \cite{katsaros_performance_2011} were the first to employ this method for a GLOSA system. In their study, the location of each traffic light is extracted from an obtained topology message and then compared to the vehicle's position and heading. Notably, this study was conducted in a SUMO environment. However, as opposed to most other simulation studies, it acknowledged that a realistic traffic light matching method was required to employ their approach in the real world. Nonetheless, not much detail on the methodology and the accuracy of such an approach is presented.

Bernais et al. (2016) \cite{bernais_design_2016} describe a similar approach in a real-world GLOSA system for Braunschweig, Düsseldorf, and Kassel. Instead of matching an individual lane, their approach includes matching upcoming traffic lights based on their stop line. This reduces the complexity of the matching problem significantly since the vehicle's heading can be roughly compared to each ingress direction on the upcoming intersection instead of comparing the position to each individual lane. On the other hand, the stop-line method introduces the need for a multi-lane presentation of the speed advisory since no decision on a specific traffic light is made. This approach is also used by Yunex's traffic light service "APHA" according to the technical documentation \cite{yunex_traffic_v2x-kommunikation_2023}. Other studies, such as by Khan et al. (2021) \cite{khan_eco-drive_2021}, or also the TrafficPilot app, presumably use a similar approach due to their multi-lane visualization. Yet, it is not entirely clear if this assumption is true due to the lack of methodological details and reported results. Thus, it is difficult to reproduce or judge the accuracy of this approach.

Wilson et al. (2017) \cite{wilson_driver_2017}, developing BMW's EnLighten system, studied how a smartphone's inbuilt GNSS sensors could be utilized to determine the upcoming traffic light. Not much is known about the traffic light matching methodology, again. However, this study contributes to understanding which challenges a location-based approach may impose in real-world deployments. Wilson et al. (2017) \cite{wilson_driver_2017} were the first to report issues with the smartphone's GNSS. This is a significant problem since the location-based approach assumes that the GNSS is accurate enough and always available to determine the vehicle position on a lane. However, in their study, the GLOSA system would regularly shut itself down due to losing GNSS connectivity. As the authors reported, this problem leads to user frustration and a degraded System Usability Score, depending on the malfunctioning rate. How the authors came to the conclusion that the system's GNSS connectivity must have been the issue and not, for example, its accuracy, was not clarified. Nonetheless, it is clear that we cannot simply assume a perfect GNSS availability. 
...
Besides GNSS inaccuracies, Stahlmann et al. (2018) \cite{stahlmann_exploring_2018} also pointed out one other factor that is crucial for the location-based approach to work well: long enough lane geometries on the intersection topology. In their work, this parameter is described under the term link length being between 590 to 910 meters, allowing the system to determine a suitable lane early on. The link is drawn centrally on the ingress car lanes toward the corresponding traffic light, thus relying on the fact that cars typically approach an intersection in one direction. This represents a significant limitation for applying this approach to cyclist traffic lights, which are typically associated with multiple ingress directions, for example, via two ingress footpaths shared between cyclists and pedestrians. The same applies to the possible turning directions at each intersection. Although trajectories can be predicted to some extent \cite{rudenko_human_2020}, one key limitation of this method is that it does not capture the planned turns that the user will make.

Due to the issues of GNSS inaccuracies and a lack of knowledge of which turn the user will make, location-based approaches are likely not a good option for cyclists. Both problems can only be addressed to a limited extent with GNSS error correction and trajectory prediction. Another challenge may be the shortness of bike lanes, likely leading to an erroneous or delayed matching. With rough matching approaches based on stop lines at intersections, the problem arises that multiple lanes must be visualized through the user interface. This, in turn, may lead to motorists' abuse of the bike-GLOSA app and additional cognitive load, making it less attractive in the context of this work. In general, many studies lack the methodological specificity required to fully understand and reproduce the proposed location-based lane-matching approach. Apart from reported challenges associated with a location-based matching approach, there are no reports about the measured per-traffic-light accuracy. 
...
In summary, to provide a speed advisory, it is not only mandatory to have a traffic light's prediction but also to identify which traffic light the user currently travels towards. To solve this problem, vision-based detection is not an option, mainly due to the smartphone's carrying or mounting spots. Alternatively, one could utilize the user's location and current heading to look up nearby traffic lights that align with the user's direction. This approach was identified as the predominant strategy in related work. However, as discussed, GNSS inaccuracies and accurately predicting the upcoming traffic light remain core challenges of the approach. 

Location-based approaches are likely more suited for multi-lane speed advisory in which a coarse-grained matching suffices. The problem is that multi-lane speed advisory also comes with the chance of the system being misused by motorists, and an increased cognitive load potentially leading to distraction. Thus, it is desirable to develop a single-lane traffic light matching method. There is a clear research gap in the development of a route-based matching method, which is an underinvestigated but promising approach for a bicycle application of GLOSA. The aim of the following concept is to address this knowledge gap and develop such a method.

\section{Concept}

A novel approach for route-based traffic light matching is proposed. The approach was published as peer-reviewed research papers at IEEE ISC2 2022 \cite{matthes2022matching} and ACM SIGSPATIAL 2023 [in print].

Route-based matching is similar to location-based matching as it assumes the route geometry as an accurate proxy for the user's trajectory. However, the route's turns can be utilized to infer which specific order of traffic lights will be taken at an intersection. Moreover, not only proximal but all traffic lights along the route can be matched in advance. This provides the foundation for displaying the speed advisory quality along the route and ensures that the speed advisories are displayed as early as possible. Furthermore, route-based traffic light matching diminishes any problems in previous works associated with GNSS inaccuracies. The goal is to find a method that accurately matches a few from thousands of traffic lights along a route geometry. To enable the speedometer visualization that only focuses on one traffic light at a time, the matching method must be accurate enough to match individual traffic lights at an intersection, not only parallel-aligned lanes that roughly align with the route. Of course, the method must also be computationally efficient to enable quick traffic light matching every time a new route is calculated by the user or through automated rerouting.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{images/sg-selection-example.png}
\caption{In this case, the OpenStreetMap route does not accurately depict the cyclist's most optimal path and runs over car traffic light 1. In reality, the bike path and four bike/pedestrian traffic lights to the right of the route are utilized: 2, 3, 4, and 5. Source: [in print]}
\label{fig:sg-selection-example}
\end{figure}

As a data foundation for the proposed method, intersection (MAP) topologies are utilized that are provided through the same platform as the real-time traffic light data. For each signal, a geographical linestring is given that shapes the lane associated with the signal. Which type of traffic is served by the traffic light is also given. The result is many traffic light geometries that roughly align with the route. Depending on the routing foundation (OpenStreetMap or DRN), the discrepancy between traffic light geometries and the calculated route can vary. This means that it is not possible to simply look up the traffic light geometries that perfectly align with the route. What's needed instead is some kind of human-like spatial reasoning that deduces which traffic lights will most probably be used by the cyclist. The following sections will explore how human-like spatial reasoning can be reflected through a ground truth and how different methods perform against this ground truth. Finally, the best method is selected and integrated into the system.

\subsection{Ground Truth}

The goal is to develop a matching procedure that correctly matches as many traffic light geometries as possible to each route. However, it is first important to deeply understand what a "correct" match means. In theory, a cyclist can sometimes traverse an intersection in multiple legal ways. Thus, which traffic light is actually chosen may depend on contextual circumstances, personal preference, or even spontaneity. Here, human reasoning can be applied to find the most likely traffic lights as "correct" matches for each intersection traversal. This assumes that cyclists will follow certain movement patterns at each intersection, and these movement patterns are sufficiently predictable by looking at the intersection layout. The idea is to hand-craft a ground truth with combinations of routes and traffic lights predicted by a human to match each given route. In the second step, this ground truth can be utilized to test models for automated traffic light matching.
...
To create a ground truth, there are multiple options. For example, one could physically ride through Hamburg along randomized routes and note down the most likely utilized signals. However, to generate a sufficiently large dataset with enough examples, an adequate number of rides throughout the city would be necessary. Ideally, the ground truth could be generated in a less time-consuming way without the need to travel to the city of Hamburg or any other remote city in which the application is deployed. The proposed tool "Route Composer" delivers this possibility. Instead of physically driving through Hamburg, the Route Composer is a web application that makes it possible to travel virtually along randomly generated routes and label the correct signals. As shown in \Cref{fig:sg-selection-ground-truth}, the Route Composer highlights the signals' and the route's geometries on a base map. The traffic lights can be selected by clicking on them in the web application. Once a route is finished, the selected traffic lights are persisted for model training and evaluation.

\subsubsection{Mitigation of Personal Preference}

As discussed earlier, it is important to select the most likely used order of traffic lights at each intersection. However, the selection in the Route Composer may be biased by limited knowledge of the intersection or personal preference, potentially leading to selections that don't represent the most likely choice. To avoid these false selections as much as possible, two measures are employed: First, the Route Composer provides the option to open the current intersection in Google Maps, which provides additional satellite imagery. The goal is to resolve ambiguous situations in which the base map or the geometries' shapes are not enough to make a reasonable decision. This measure assumes that the composing is done carefully enough to notice an ambiguous situation. The second measure is a clear ruleset that determines how to handle specific situations. As stated in [in print], the ruleset consists of seven rules:
...
The result is a dataset of random routes with their associated signals. In the following sections, we will utilize the dataset to test different methods toward the goal of an automated traffic light matching procedure.

\subsubsection{Benchmark}

To compare different matching procedures, a benchmark on the ground truth was developed. For each route in the ground truth, the matching procedure is executed and produces a list of matched signals. Then, this list of traffic lights is compared to the ground truth. The result is a number of true negatives, false negatives, true positives, and false positives. Based on these statistics, a benchmark score is calculated.

Many of the few thousand traffic lights can be excluded entirely by applying a rough distance threshold around the route. This means that, for each route, a large number of true negatives is expected. These easily excluded traffic lights are not of interest here. What's of interest are the traffic lights near the route for which a more thorough decision is required. Thus, a metric is required that ideally excludes true negatives. The F1 score is chosen here since it only considers true positives (TP), false positives (FP), and false negatives (FN): 
...
Here, a low precision is associated with a high level of speculativity in the approach, indicating its tendency to select additional traffic lights that may not match the intended route. On the other hand, a low recall metric is linked to an overly high specificity of the approach, indicating how strictly it selects signals. The F1 score represents the harmonic mean between both semantic measures. Assuming that especially false-positive or false-negative matches limit the speed advisory's perceived usability, the F1 score is suitable to accurately depict the perceived quality of the matching.

\subsection{Discarded Approaches and Learnings}
...
The core problem for route-based traffic light matching is the imperfect alignment of the route's geometry with the signals' geometries. Through discussion with colleagues, it was discovered that they solved a similar problem before. In their work, the task was to match many cyclists' tracks at intersections to detect movement patterns and stopping points. However, GNSS inaccuracies and movement variations in the tracks represented a problem during the clustering process. In other words, the user tracks did not align perfectly, similar to the imperfect alignment of routes' and signals' geometries. The colleagues solved this problem by clustering the tracks together using a spatial raster. Specifically, the H3 raster was used.

The H3 raster subdivides the earth primarily into hexagonal cells\footnote{Strictly speaking, the H3 raster has 12 additional pentagons on every level to complete a seamless shape. See: \url{https://h3geo.org/docs/core-library/restable/}}. On the top resolution level (1), there are 122 cells covering the earth's surface. On level 2, the 122 cells are subdivided into 842 cells, and so forth. As a result, the H3 model contains 15 levels, which are hierarchically connected in a tree structure. Through this hierarchical structure, it is possible to efficiently navigate through the index, e.g., to find neighbor cells. After the GNSS samples of each user track were assigned to an H3 cell, the resulting cells each contain a number of samples and travel direction (edge of the H3 cell). These spots indicate the stopping points. Due to the size of the raster, minor GNSS errors or individualities in the user movement don't fall into weight as they are clustered together. Since this approach was described as remarkably successful, a similar strategy was adopted for traffic light matching.
...
In every case, the H3 approach assumes that the route geometry is near enough to the matching traffic light geometries. However, many false negatives were produced when the route was mapped on the road, while the most likely chosen traffic light was a few meters to the side of the road. A potential solution to this problem would be to choose a larger raster, but this would worsen the score even more since many signals' geometries are no longer captured adequately. Instead, they would resemble a circular "blob", as highlighted on the left side of \Cref{fig:sg-selection-h3-example}.
...
Due to these issues, the H3-based approach for traffic light matching was discarded. The takeaway from this approach is that close alignment does not sufficiently model matching traffic lights along a route geometry. Instead, sometimes, the traffic light geometries may be offset from the route, especially if the route is snapped onto the car lanes.

\subsubsection{Graph Approach}

The second approach is inspired by the map-matching procedure proposed by Newson and Krumm in 2009 \cite{newson_hidden_2009}. Their proposed method matches GNSS trajectories to the road network by building a transition graph between each GNSS sample and the nearby roads. After the graph is created, each transition is marked with a transition probability. In their approach, the most probable path through the transitions is calculated using the Viterbi algorithm to find the sequence of roads that were taken. 

Similar to the problem of map-matching, the idea is to find the most probable pathway over the traffic light geometries to traverse an intersection from ingress to egress. The traffic light geometries are handled as "road" segments, and the route geometry represents the GNSS trajectory. 
...
The proposed approach operates as illustrated in \Cref{fig:sg-selection-graph-approach}. First, traffic lights along the route are prefiltered using a rough distance threshold of 20m. The remaining traffic lights are clustered into intersections based on the intersection ID given in the signal's metadata. Around each of these intersections, the procedure undergoes three steps: (1) A bounding box is calculated with an additional padding. The route enters and exits the bounding box at one point. (2) A fully connected graph is generated between the route's entry and exit points and each traffic light geometry's start and end points. (3) A cost is assigned to each transition in the graph based on the transition's length. Finally, the least costly path is calculated using the Dijkstra pathfinding algorithm. The idea is that the least costly path represents the most likely utilized trajectory over the traffic light geometries.

The cost function is defined to penalize traversal between signals, and traversal using the traffic lights is preferred. Otherwise, the most optimal path would always be the direct connection between the entry and exit points. Specifically, every transition in the graph  $(p_{1} \rightarrow p_{2})$ has the cost $dist(p_{1}, p_{2})$, while transitions between traffic lights have the cost $dist(p_{1}, p_{2}) \times \text{penalty}$. Thus, the penalty value represents a hyperparameter of the model together with the dimensions of the intersection padding. The Tree-Structured Parzen Estimator hyperparameter tuning algorithm \cite{ozaki_multiobjective_2020} is utilized to find the best possible configuration. This tuner considers each hyperparameter (intersection padding and penalty) independently from each other. During the tuning, the model undergoes inference for all routes in the dataset (one trial). After each of these trials, the F1 score is evaluated, and the hyperparameters are adjusted by the tuner.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{images/sg-selection-graph-example.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{images/sg-selection-graph-strict.png}
...
Although the graph approach initially promised good results, it was ultimately abandoned in favor of other ideas. Even if the graph approach seems less susceptible to offsets and longer traffic light geometries, it clearly falls short when considering the curvature and not only the absolute location of the traffic light geometries. In the end, only each traffic light geometry's start and end points are considered. However, the curvature is similarly important to decide which traffic light could match the associated route. A positive learning of this approach is that the map-matching approach of Newson and Krumm (2009) \cite{newson_hidden_2009} could be in theory a good solution for the traffic light matching problem.

\subsubsection{Map-Matching Approach}

In an ideal scenario, each matching traffic light geometry would perfectly align with the route's geometry. However, as highlighted before, this is typically not the case. Often, the generated route is offset from the most likely utilized signal. Of course, during the intersection planning process, it is not of interest if the signal's geometry would later align with the OpenStreetMap topology or other routing foundations. Nonetheless, the question arises: Since there is no option to intervene directly in the traffic light geometry creation process, would it be possible to adjust the traffic light geometries to the utilized routing foundation as a preprocessing step?
...
The matching procedure designed as part of a publication at ISC2 2022 \cite{matthes2022matching} aims to tell apart matching from non-matching traffic light geometries by a geometrical comparison to the route. In four filtering steps specific geometric thresholds are applied to filter out traffic lights that are likely no match. The idea is that each step catches a specific type of misalignment with the route, such that in the end, only the wanted matches remain. In the next sections, we will first look at the workwise of each filter. Afterward, it will be discussed how the filter thresholds are optimized.

\subsubsection{Distance Filter}

Step one focuses on coarsely excluding a large proportion of the traffic lights in the city area through a distance threshold $t_{dist}$ from the route. This step is efficiently executed through a PostGIS database using the \texttt{ST\_Distance}\footnote{\url{https://postgis.net/docs/en/ST\_Distance.html}} query.

\subsubsection{Bearing Filter}

As a next step, the bearing between the signal's geometry and the route is compared. Hereby, we can exclude traffic lights that are angled too steeply or even inverted to the route. As a foundation, the atan2 function is used to calculate the bearing between two coordinates on the earth\footnote{\url{https://www.movable-type.co.uk/scripts/latlong.html}}:
...
\toprule
Hyperparameter  & Explanation & Search Interval \\
\midrule
$t_{dist}$ & Traffic light geometries must be within this distance of the route. & $[1m, 100m]$ \\
\midrule
$t_{bear}$ & If segments of the traffic light geometry are angled more steeply to the route than this value, they are not counted for $t_{bear\_sum}$. & $[0°, 180°]$ \\
$t_{bear\_sum}$ & The minimum proportion of the signal's geometry matching $t_{bear}$. & $[0, 100\%]$ \\
\midrule
$t_{len}$ & If segments of the traffic light geometry are elongated or shortened more than $1 \pm t_{len}$ when projected onto the route, they are not counted for $t_{len\_sum}$. & $[0, 1]$\\
$t_{len\_sum}$ & The minimum proportion of the signal's geometry matching $t_{len}$. & $[0, 100\%]$ \\
\midrule
$t_{overlap\_pct}$ & If two traffic lights overlap more relative to their combined length, a decision process between both traffic lights is initiated. & $[0, 100\%]$ \\
$t_{perfect\_match}$ & If a signal's geometry is closer to the route than this value, it is preferred over another overlapping traffic light geometry. & $[0, 50m]$ \\
$t_{road\_side}$ & If two overlapping traffic lights are on opposite sides of the route and less than this distance away from each other, decide by the ordinary traffic side. & $[0, 100m]$ \\
\bottomrule
\end{tabular}
\label{tab:hyperparameter-space}
\end{table}

\subsubsection{Model Finetuning}

The designed filtering pipeline has several thresholds that must be carefully selected to optimize the final matching performance. Here, it is important to consider that each filter influences the subsequent filters in the pipeline. This means that it is difficult and time-intensive to tune the individual filters by hand. A solution to this problem is utilizing a hyperparameter tuning algorithm, which tries to understand the influence of each hyperparameter on a model and maximizes its score. This algorithm can run overnight, resulting in a proposed model configuration after a few hundred to thousand trials. To prepare the model for hyperparameter tuning, search spaces for each hyperparameter must be defined. These search spaces are summarized in \Cref{tab:hyperparameter-space}.

The Tree-Structured Parzen Estimator hyperparameter tuning algorithm \cite{ozaki_multiobjective_2020} is utilized to find a good configuration on the ground truth benchmark. The objective function given to the tuner algorithm is the F1 score. 

\begin{table}[h]
\caption{Final thresholds of the designed filtering pipeline for traffic light matching.}
\begin{tabular}{@{}lllllllll@{}}
\toprule
  Routing & $t_{dist}$ & $t_{bear}$ & $t_{bear\_sum}$ & $t_{len}$ & $t_{len\_sum}$ & $t_{road\_side}$ &  $t_{perfect\_match}$ & $t_{overlap}$ \\
  \midrule
  OSM & 20m & 32.64° & 78.63\% & 0.999 & 92.65\% & 58.97m & 49.99m & 42.70\% \\
  DRN & 17m & 26.75° & 73.43\% & 0.813 & 60.76\% & 47.87m & 42.51m & 58.52\% \\
\bottomrule
\end{tabular}
\label{tab:hyperparameter-tuning-results}
\end{table}

Since two routings are utilized (OpenStreetMap and DRN), coming with different alignments of the generated routes to the traffic light geometries, the model must be tuned separately on both datasets. Therefore, one ground truth is created for each routing foundation. The final model configuration for both routing foundations is highlighted in \Cref{tab:hyperparameter-tuning-results}. Here, it is visible that the DRN version of the filtering pipeline has slightly more strict thresholds than the OpenStreetMap version. This is expected as the goal of the DRN routing foundation is to provide a more accurate bike routing, which is supposed to more accurately align with the traffic light geometries. A thorough performance study of this approach is given in \Cref{ch:results}.

\subsection{Machine Learning Filtering Approach}

One identified problem of the algorithmic approach is that each threshold is defined globally and equally applied to all situations. However, not all intersections are crossed similarly by routes. Thus, the filtering pipeline would be applied too strictly in some situations, while the thresholds are not applied strictly enough in others. Here, a machine learning model can be useful to employ a more flexible and interlinked mapping of the geometric properties. Such a model was developed under supervision as part of Daniel Jeschor's Master thesis \cite{jeschor_2022} and published at ACM SIGSPATIAL 2023 [in print].

Regarding the problem of deciding between "match" and "no match" for each traffic light geometry, there may be many creative ways to establish a machine learning classifier. For example, it would be possible to generate top-down images of each intersection and utilize machine learning techniques from image detection or segmentation to classify each signal. However, as a rule of thumb, more complicated tasks tend to require larger machine learning models, which in turn require a larger ground truth. Unfortunately, the ground truth itself would contain a few thousand samples at most, which speaks against the viability of applying more advanced deep learning models. Dataset augmentation through artificial distortion of the traffic light geometries was found in preliminary experiments not to be a purposeful option. A meaningful balance between marginally distorting the traffic light geometries and invalidating the labeling could not be identified. Furthermore, there would be the concern of scalability with larger models and a more complex feature processing pipeline. Thus, the decision was made to opt for a more conservative approach.
...
\subsubsection{Feature and Model Grid Search}

To select a suitable machine learning model architecture and feature processing pipeline, experiences from a previous publication at IoTaIS 2022 \cite{matthes2022selecting} were utilized to systematically and rigorously identify promising configurations. Out of the tested traditional model architectures k-Nearest-Neighbors (k-NN), Decision Tree, Random Forest, Multilayer Perceptron (MLP), AdaBoost, Gaussian Naive Bayes, Quadratic Discriminant Analysis (QDA), and Support Vector Machine (SVM), the MLP model overall performed the best and was thus selected as the final classifier, after testing other promising models with different feature constellations. 
...
As in the algorithmic approach, there may be some cases where multiple overlapping traffic lights are selected. These duplicate selections may happen when multiple traffic light geometries are very closely aligned to the route, and must be cleaned up, as the app should only focus on the most likely used signal. 
...
Not all intersections have traffic lights connected to Hamburg's data broker. However, this represents a problem for matching traffic lights along a route. If an unconnected intersection lies between the user and the next connected intersection, the user may get a speed advisory for the wrong intersection. Although the traffic light is visualized on the in-app map, this may still lead to confusion for the user. Thus, it is crucial also to identify the unconnected intersections along the bike path. These unconnected intersections are visualized with a unique traffic light symbol in the user interface.

To find out which intersections are not connected to the system (but still signalized), the Lichtsignalanlagen Hamburg\footnote{\url{https://metaver.de/trefferanzeige?docuuid=C498DEED-985C-11D5-889E-000102B6A10E}} dataset is utilized. This dataset is updated periodically every four months and contains single coordinates of each signalized intersection in Hamburg (see \Cref{fig:sg-selection-unconnected-intersections}). Thus, the dataset does not resolve individual traffic light geometries but the center locations of the intersection node. 

To identify which intersections are connected, it is determined which intersections have traffic light geometries within an empirically defined threshold of 50 meters. These intersection nodes from the Lichtsignalanlagen Hamburg dataset are marked as "connected." All other nodes are marked as "unconnected." In case of a traffic light unassigned to any intersection node, it is assumed that this intersection node is missing in the Lichtsignalanlagen Hamburg dataset, and a new intersection node is created. Finally, a list of intersection nodes is matched to the route by a distance threshold of 50 meters. To avoid displaying a speed advisory for a wrong intersection, the app handles each unconnected intersection like a traffic light that doesn't send any data. Once an unconnected intersection is passed before another (connected) signal, the speed advisory is enabled again. This step completes the traffic light matching pipeline.

\subsection{Summary of Methods for Traffic Light Matching}
...
Finally, to avoid speed advisories being given for a traffic light that comes after an unconnected but signalized intersection, the Lichtsignalanlagen Hamburg dataset is utilized. The implemented web service, which incorporates both final models and matches unconnected intersections, is accessible to the app through a web API.

\section{Results}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\linewidth]{images/lane-analysis-lane-lengths.pdf}
\caption{.}
\label{fig:}
\end{figure}
...
\begin{figure}[htbp]
\centering 
\includegraphics[width=0.75\linewidth]{images/binding-coverage-hamburg.png}
\caption{.}
\label{fig:}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\linewidth]{images/coverage-analysis.pdf}
\caption{.}
\label{fig:}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\linewidth]{images/coverage-analysis-2.pdf}
\caption{.}
\label{fig:}
\end{figure}
...
\begin{figure}[htbp]
\centering 
\includegraphics[width=0.75\linewidth]{images/model-analysis.pdf}
\caption{.}
\label{fig:}
\end{figure}
...
\centering
\begin{tabular}{cc}
\footnotesize{(a) False matches} & \footnotesize{(b) Correct matches} \\
\includegraphics[width=0.35\linewidth]{images/error-analysis-constellations-false.pdf} & \includegraphics[width=0.35\linewidth]{images/error-analysis-constellations-right.pdf} \\
\end{tabular}
\caption{.}
\label{fig:}
\end{figure}
