...
Afterward, we address the challenge of identifying traffic lights cycled toward. A literature study finds that existing camera-based approaches are impractical for cyclists, and location-based approaches face significant issues with mismatching due to geolocation inaccuracies. A novel route-based approach is proposed that selects the best traffic lights, comparing their turn geometries with the bike route's shape. However, establishing such an approach proves challenging in many situations where the bike route generated based on OpenStreetMap does not accurately capture the actual cycling infrastructure. Out of 1,035 considered cases, only 23\% are error-free, with 72\% of error cases resulting from routing in the center of the road instead of the appropriate bike path. Thus, spatial reasoning is found to be required to determine the best unambiguous sequence of traffic lights that will be passed by the cyclist at an intersection. Several approaches are tested, ultimately arriving at a machine learning model that achieves matching F1 scores of 92\% and 86.2\% validated on a separate dataset. Compared to previous online approaches, this method also provides the advantage of matching all traffic lights in advance, requiring a median extra time of 1.4 seconds during bike route calculation. Together with transmitting the prediction via mobile internet, this maximizes the possible speed advisory activation distance.

Although the developed matching model produces a correct match in most cases with routing errors, a more accurate bike routing than possible with OpenStreetMap is desirable to enhance the traffic light matching accuracy. Furthermore, it is identified that
  \end{abstract}
